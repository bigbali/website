...
export class ScrollObserver implements Observer {
    ...
    constructor(
        ...
        this.#observer = new IntersectionObserver(
            (elements) => callback(elements, this),
            options
        );
    }
    ...
    observe(query: string) {
        document
            ...
            .forEach((element) => this.add(element as HTMLElement));
    }
...
const scrollAnimationCallback: ScrollObserverCallback = (
    elements,
    observer
) => {
    elements.forEach((element) => {
        if (
            element.target.getAnimations().length === 0 &&
            element.isIntersecting
        ) {
            const animationEnd = () => {
                element.target.classList.add('animation-finished');
                element.target.removeEventListener(
                    'transitionend',
                    animationEnd
                );
